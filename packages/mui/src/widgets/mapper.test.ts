...
describe("mui mapper", () => {
  test("merges logical props and mui native options, ignoring other namespaces", () => {
    const mapped = mapMuiProps(input());
  ...
  test("rejects protected keys and handlers", () => {
    ...
    const field = {
      ...base.field,
      native: { mui: { defaultValue: "x", onChange: () => undefined } },
    };
    expect(() => mapMuiProps(input({ field }))).toThrow(RendererAdapterError);
    try {
      mapMuiProps(input({ field }));
    } catch (error) {
