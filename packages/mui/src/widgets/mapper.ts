...
export const MUI_NAMESPACE = "mui";
...
export function mapMuiProps(input: WidgetRenderInput): Readonly<Record<string, unknown>> {
  const merged: Record<string, unknown> = {};
  collectLayer(merged, input.field.props, input, "props");
  collectLayer(merged, input.field.native?.[MUI_NAMESPACE], input, "native");
...
function protectedKeyError(input: WidgetRenderInput, key: string, layer: string): RendererAdapterError {
  const diagnostic: Diagnostic = {
    ...
    source: "adapter",
    pluginId: MUI_NAMESPACE,
    modelPath: input.field.path,
    metadata: {
      adapterId: MUI_NAMESPACE,
