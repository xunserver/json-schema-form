...
describe("antd mapper", () => {
  test("merges logical props and antd native options, ignoring other namespaces", () => {
    const mapped = mapAntdProps(input());
  ...
  test("rejects protected keys and handlers", () => {
    ...
    const field = {
      ...base.field,
      native: { antd: { defaultValue: "x", onChange: () => undefined } },
    };
    expect(() => mapAntdProps(input({ field }))).toThrow(RendererAdapterError);
    try {
      mapAntdProps(input({ field }));
    } catch (error) {
