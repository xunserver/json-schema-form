...
export const ANTD_NAMESPACE = "antd";
...
export function mapAntdProps(input: WidgetRenderInput): Readonly<Record<string, unknown>> {
  const merged: Record<string, unknown> = {};
  collectLayer(merged, input.field.props, input, "props");
  collectLayer(merged, input.field.native?.[ANTD_NAMESPACE], input, "native");
...
function protectedKeyError(input: WidgetRenderInput, key: string, layer: string): RendererAdapterError {
  const diagnostic: Diagnostic = {
    ...
    source: "adapter",
    pluginId: ANTD_NAMESPACE,
    modelPath: input.field.path,
    metadata: {
      adapterId: ANTD_NAMESPACE,
