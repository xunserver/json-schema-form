...
describe("element-plus mapper", () => {
  test("merges logical props and element-plus native options, ignoring other namespaces", () => {
    const mapped = mapElementPlusProps(input());
  ...
  test("rejects protected keys and handlers", () => {
    ...
    const field = form.model.ui.fields.get("name")!;
    expect(() => mapElementPlusProps(input({ field }))).toThrow(RendererAdapterError);
    try {
      mapElementPlusProps(input({ field }));
    } catch (error) {
