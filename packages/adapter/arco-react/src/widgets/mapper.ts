...
export function mapArcoReactProps(input: WidgetRenderInput): Readonly<Record<string, unknown>> {
  const merged: Record<string, unknown> = {};
  collectLayer(merged, input.field.props, input, "props");
  collectLayer(merged, input.field.native?.[ARCO_REACT_NAMESPACE], input, "native");
...
function protectedKeyError(input: WidgetRenderInput, key: string, layer: string): RendererAdapterError {
  const diagnostic: Diagnostic = {
    ...
    source: "adapter",
    pluginId: ARCO_REACT_NAMESPACE,
    modelPath: input.field.path,
    metadata: {
      adapterId: ARCO_REACT_NAMESPACE,
