# @xunserver-jsf/arco-react

Arco Design React UI Adapter，位于 React Renderer 之上。

```bash
pnpm add @xunserver-jsf/core @xunserver-jsf/react @xunserver-jsf/arco-react
pnpm add react @arco-design/web-react
```

导出 `arcoReactAdapter`、`createArcoReactAdapter()`、`extendArcoReactAdapter()`。`react` 与 `@arco-design/web-react` 是 peer dependency。

文档：https://xunserver.github.io/json-schema-form/
