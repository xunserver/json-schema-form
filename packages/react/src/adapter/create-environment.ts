...
import { createReactRegistry } from "./registry.js";
import type {
...
export function createReactRendererEnvironment(
  options: CreateReactRendererEnvironmentOptions,
): ReactRendererEnvironment {
    ...
    if (!snapshot.validShape) {
      tracked.push(
        track({
          ...
          message: "ReactUIAdapter must be a plain object",
  ...
  for (const snapshot of firstById.values()) {
    ...
    if (!isProtocolCompatible(compatibility)) {
      tracked.push(
        track({
          code: RENDERER_DIAGNOSTIC_CODES.PROTOCOL_INCOMPATIBLE,
          message: `Adapter "${snapshot.id}" is incompatible with React renderer protocol ${formatProtocol()}`,
          ordinal: snapshot.ordinal,
          pluginId: snapshot.id,
          metadata: {
  ...
  const widgetStores = new Map<string, Map<string, ReactRegistryEntryInspection<WidgetBinding>>>();
  const layoutStores = new Map<string, Map<string, ReactRegistryEntryInspection<LayoutBinding>>>();
  const consumedOverrides = new Set<ReactRegistryOverride>();

  for (const snapshot of firstById.values()) {
    if (isRecord(snapshot.widgets)) {
      const widgets = new Map<string, ReactRegistryEntryInspection<WidgetBinding>>();
    ...
    if (isRecord(snapshot.layouts)) {
      const layouts = new Map<string, ReactRegistryEntryInspection<LayoutBinding>>();
  ...
  const resolved: ResolvedReactAdapter[] = [];
  for (const snapshot of firstById.values()) {
    ...
    resolved.push(
      Object.freeze({
        id: snapshot.id,
        protocol: REACT_RENDERER_PROTOCOL,
        form: snapshot.form as FormAdapter,
        fieldChrome: snapshot.fieldChrome as FieldChromeAdapter,
        widgets: createReactRegistry(snapshot.id, "widgets", [...widgets.values()]),
        layouts: createReactRegistry(snapshot.id, "layouts", [...layouts.values()]),
      }),
    );
  }

  const byId = new Map(resolved.map((adapter) => [adapter.id, adapter]));
  const environment: ReactRendererEnvironment = {
    protocol: REACT_RENDERER_PROTOCOL,
...
function registerBindings<T>(
  ...
  store: Map<string, ReactRegistryEntryInspection<T>>,
  overrides: readonly ReactRegistryOverride[],
  consumedOverrides: Set<ReactRegistryOverride>,
...
function snapshotAdapter(adapter: ReactUIAdapter, ordinal: number): AdapterSnapshot {
  if (!isRecord(adapter)) {
    return {
      ordinal,
      validShape: false,
...
function isWildcardOverride(override: ReactRegistryOverride): boolean {
...
function freezeVersion(version = REACT_RENDERER_PROTOCOL) {
  return Object.freeze({ major: version.major, minor: version.minor });
}

function formatProtocol(): string {
  return `${REACT_RENDERER_PROTOCOL.major}.${REACT_RENDERER_PROTOCOL.minor}`;
}
