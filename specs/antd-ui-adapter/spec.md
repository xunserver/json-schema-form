# antd-ui-adapter Specification

## Purpose

为 React Renderer 提供官方 Ant Design UIAdapter，把九种逻辑 Widget、布局、FieldChrome 和语义交互映射到 Ant Design，同时维持 Core canonical values、validation 真相与 accessibility 边界。
...
- **WHEN** 选择 `antd` adapter 执行 preflight
- **THEN** 每个 logical key 唯一解析到兼容 binding

### Requirement: props 与 native mapper 保护 Core-owned 键
mapper 必须（MUST）只读取 `native["antd"]`，并保护 value/disabled/ARIA/handler 等 Core-owned 键。

#### Scenario: 其他 namespace 被忽略
- **GIVEN** 同一 Field 含 `antd` 与其他 adapter native options
