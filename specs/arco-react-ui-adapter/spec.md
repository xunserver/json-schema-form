...
`@xunserver-jsf/arco-react` 必须（SHALL）公开一个冻结且具有稳定 ID `arco-react` 的 UIAdapter，组合 FormAdapter、FieldChromeAdapter、WidgetAdapterRegistry 与 LayoutAdapterRegistry。FormAdapter 必须（MUST）只作为结构/样式容器，不得（MUST NOT）把 UI library form model、validation 或 DOM validity 当作业务 values、errors 或 transaction 真相。
