...
- **GIVEN** 外部消费者导入未声明的Core compiler、schema frontend、rule、dynamics、validation engine、scheduler、run-state、runtime、transaction、store、array binding、view state、adapter或engine implementation路径
...
- **GIVEN** 外部消费者尝试从root、runtime或extension入口取得Rule/activation/serialization/validation writer、mutable dependency graph、run-token mutator、binding table、Compiler Context、RuntimeNodeId或其他等价escape hatch
