...
打开 http://127.0.0.1:5173/ 。左侧五个 JSON 编辑器对应 `FormDefinition` 的 `schema` / `uiSchema` / `rules` / `config` 以及 `createForm` 的 `initialValues`（formData）。右侧是 MUI `FormRenderer` 预览与 Inspector（诊断、live values、`serialize()`、submit payload）。顶栏 Example 切换共享 catalog；「Element Plus」链接跳到 Vue playground（默认 http://127.0.0.1:5174/?example=...），不会在同一页面挂载 Vue。
